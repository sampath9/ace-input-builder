...
<body bgcolor="#FCFAE8">
	<form  name="InitialConditions" id="InitialConditions" method="post" onsubmit="fnGetValues();">
			...
			<tr>
				<td>
					...
					<li class="dropdown"><a href="Main.jsp">Main</a>
						...
						<li class="dropdown"><a href="Wethaer.jsp">Weather</a>
							<!-- <ul class="sub_navigation">
								<li><a href="#">Sub Navigation 1</a></li>
								<li><a href="#">Sub Navigation 2</a></li>
								<li><a href="#">Sub Navigation 3</a></li>
					...
					<table cellpadding="20">
						<tr>
						    ...
						    SortingValuesUsingjsp svuj = (SortingValuesUsingjsp)session.getAttribute("objectMain");
						     session.setAttribute("visited", "initialConditions");
						     //session.setAttribute("objectMain", svuj);
						     int CountInitConditions = svuj.getInitConditionsValuesCounter();
						     //out.print(CountInitConditions);
						     //out.print(svuj.getInitialConditions());
						     String InitialConditions[][] = new String[CountInitConditions-1][2];
						     InitialConditions = svuj.getInitialConditions();%>
						     <select name="codeDisplay" id="codeDisplay" style="border-color: #FFD175;">
						     <%for (int i=0; i<=CountInitConditions-1; i++){%>
									<option value="<%= InitialConditions[i][0] %>"><%= InitialConditions[i][0] %></option>
						     	<%}
						     	%>
						     </select></td>
							<td>
							...
							<select name="Description" id="Description" style="border-color: #FFD175;">
						     <%for (int i=0; i<=CountInitConditions-1; i++){%>
									<option value="<%= InitialConditions[i][1] %>"><%= InitialConditions[i][1] %></option>
						     	<%}%>
						     </select></td>
							<td>
							Unit or Type
							<select name="unit_or_type" id="unit_or_type" style="border-color: #FFD175;">
						     <%for (int i=0; i<=CountInitConditions-1; i++){%>
									<option value="<%= InitialConditions[i][2] %>"><%= InitialConditions[i][2] %></option>
						     	<% } %>
						     </select></td>
